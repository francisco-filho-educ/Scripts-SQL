...
case when cd_etapa_sige in (121,122,123,124,125,126,127,128,129,183,162,184,188,163,185,189,164,186,190,165,187,191,180,181)
      									             then 'Ensino Regular'
when cd_etapa_sige in (213,214,195,194,175,196,174,173)  then 'EJA' else 'Não se aplica' end ds_oferta_ensino,
...
case when cd_nivel = 28 then 'Educação Infantil'
     ...
     when cd_nivel = 27 and cd_etapa_sige<>137 then 'Ensino Médio' else 'Não se aplica' end ds_etapa_aluno,
...
case when cd_etapa_sige in (121,122,123,124,125,172,194) then 'Anos Iniciais'
     ...
     when cd_etapa_sige  = 175                           then 'Anos Iniciais e Anos Finais' else 'Não se aplica' end ds_subetapa,
case 
...
case 
     when cd_etapa_sige = 121 then ' 1º Ano'
     when cd_etapa_sige = 122 then ' 2º Ano'
     when cd_etapa_sige = 123 then ' 3º Ano'
     when cd_etapa_sige = 124 then ' 4º Ano'
     when cd_etapa_sige = 125 then ' 5º Ano'
     when cd_etapa_sige = 126 then ' 6º Ano'
     when cd_etapa_sige = 127 then ' 7º Ano'
     when cd_etapa_sige = 128 then ' 8º Ano'
     when cd_etapa_sige = 129 then ' 9º Ano'
     when cd_etapa_sige in(162,184,188) then '1ª Série'
     when cd_etapa_sige in(163,185,189) then '2ª Série'
     when cd_etapa_sige in(164,186,190) then '3ª Série'
     when cd_etapa_sige in(165,187,191) then '4ª Série' else 'Não se aplica' end ds_ano_serie,
cd_aluno
from aluno_etapa
)
